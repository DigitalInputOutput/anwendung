...
from system.settings import CACHE_FOLDER,STATIC_ROOT,MINIFIED_ROOT,USER,GROUP
import pwd
import grp
...
class PathBuilder:
    ...
    @staticmethod
    def generate_minified_path(current_file, files_type, device, version):
        # Remove STATIC_ROOT from the path to get the relative path
        relative_path = current_file.replace(str(STATIC_ROOT), "")

        # Construct the new path in MINIFIED_ROOT with versioning
        base, ext = os.path.splitext(relative_path)
        minified_relative_root = str(MINIFIED_ROOT) + f"/{files_type}/{device}/{version}"
